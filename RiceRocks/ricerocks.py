...
def process_sprite_group(group, canvas):
	"""Loop through set and update and draw item on canvas"""
...
def group_collide(group, other_object):
	"""Check for collision of two objects and return true if they collide."""
...
def group_group_collide(group, other_group):
	"""
	Checks each item of each group for collision and removes
	collided items from their set using discard.
	"""
	count = 0
	for item in set(group):
...
class Sprite:
	...
	def draw(self, canvas):
		# takes current center as a list for drawing
		image_center = list(self.image_center)
		if self.animated:
			# if animated image then change the x pos of the sprite for animation
			image_center[0] = self.image_center[0] + (self.image_size[0] * self.age)
...
def draw(canvas):
	...
	# draw UI here so that lives/score os not covered by onscreen items
	canvas.draw_text("Lives: " + str(lives), [50, 50], 30, "White")
	canvas.draw_text("Score: " + str(score), [620, 50], 30, "White")
...
def rock_spawner():
	...
	# keep rock_pos 100px away from ship
